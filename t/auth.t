# Copyright SUSE LLC
# SPDX-License-Identifier: GPL-2.0-or-later

use Mojo::Base -strict, -signatures;

use FindBin;
use lib "$FindBin::Bin/lib";

use Test::More;
use Test::Mojo;
use Test::Output 'stderr_like';
use Test::Warnings ':report_warnings';
use Dashboard::Test;

plan skip_all => 'set TEST_ONLINE to enable this test' unless $ENV{TEST_ONLINE};

my $dashboard_test = Dashboard::Test->new(online => $ENV{TEST_ONLINE}, schema => 'auth_test');
my $config         = $dashboard_test->default_config;

subtest 'No tokens configured' => sub {
  my $no_token_config = {%$config, tokens => []};
  my $t               = Test::Mojo->new(Dashboard => $no_token_config);
  stderr_like {
    $t->get_ok('/api/v1/incidents')->status_is(200, 'GET should still work');
    $t->patch_ok('/api/v1/incidents' => json => [])
      ->status_is(200, 'POST/PATCH/etc should work because no tokens are required');
  }
  qr/access_log/, 'access log caught';
};

subtest 'Tokens configured' => sub {
  my $t = Test::Mojo->new(Dashboard => $config);
  stderr_like {
    $t->get_ok('/api/v1/incidents')->status_is(200, 'GET bypasses token check');
    $t->patch_ok('/api/v1/incidents' => json => [])
      ->status_is(403, 'missing Authorization header')
      ->json_is({error => 'Permission denied'});
    $t->patch_ok('/api/v1/incidents' => {Authorization => 'Bearer some_token'} => json => [])
      ->status_is(403, 'invalid Authorization header format')
      ->json_is({error => 'Permission denied'});
    $t->patch_ok('/api/v1/incidents' => {Authorization => 'Token wrong_token'} => json => [])
      ->status_is(403, 'incorrect token')
      ->json_is({error => 'Permission denied'});
    $t->patch_ok('/api/v1/incidents' => {Authorization => 'Token test_token'} => json => [])
      ->status_is(200, 'correct token');

    # Extra coverage for Auth::Token branches
    $t->patch_ok('/api/v1/incidents' => {Authorization => 'Token '} => json => [])->status_is(403, 'token is empty');
    $t->patch_ok('/api/v1/incidents' => {Authorization => 'Token'}  => json => [])->status_is(403, 'token prefix only');
  }
  qr/access_log/, 'access log caught';
};

done_testing();
